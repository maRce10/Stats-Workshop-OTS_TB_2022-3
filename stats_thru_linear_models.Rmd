---
title: <center><font size="7"><b>Statistical inference using (only) linear models</b></font></center>
...
# details on simulations in R
https://aosmith.rbind.io/2018/08/29/getting-started-simulating-data/
    
# everything is a linear model
https://lindeloev.github.io/tests-as-linear/?utm_source=pocket_mylist    
...
The output is a numeric vector of length 1 thousand (`n = 1000`):
...
- Change the `mean` argument in the `rnorm()` function call ([line 78](#cb51-5)) to a value other than 0 and look how things change in the **Coefficients** table 
...
- The model tell us that the estimate of 'x1' is `r summ_xy_mod$coefficients[1, 1]` and that it's not significantly different from 0 (p-value = `r summ_xy_mod$coefficients[2, 4]`)
...
- The model tells us that $\beta_1$ (the effect size of 'x1') is `r summ_xy_mod2$coefficients[2, 1]` and that it's significantly different from 0 (p-value = `r summ_xy_mod2$coefficients[2, 4]`)
...
- The simulated values for the regression parameters can be compared to the summary of the `lm()` model to get a sense of the model precision:
    ...
    - $\beta_1$ (the effect size of 'x1') was set to `r b1` ([line 107](#cb65-4)) and was estimated as `r round(summ_xy_mod2$coefficients[2, 1], 3)` by the model  
...
- The model found that $\beta_1$ (the effect size of 'x1') is `r summ_xy_mod_multp$coefficients[2, 1]` and that it's significantly different from 0 (p-value = `r summ_xy_mod_multp$coefficients[2, 4]`)

- It also found that the $\beta_2$ (the effect size of 'x2') is `r summ_xy_mod_multp$coefficients[3, 1]` and that it's also significantly different from 0 (p-value = `r summ_xy_mod_multp$coefficients[3, 4]`)

- The simulated values for the regression parameters can be compared to the summary of the `lm()` model to get a sense of the model precision:

    - $\beta_1$ was set to `r b1` ([line 132](#cb72-4)) and was estimated as `r round(summ_xy_mod_multp$coefficients[2, 1], 3)`

    - $\beta_2$ (the effect size of 'x2') was set to `r b2` ([line 133](#cb72-5)) and was estimated as `r round(summ_xy_mod_multp$coefficients[3, 1], 3)`
...
For categorical predictors we can first create a binary (0, 1) variable and then add labels to each value. The binomial distribution (`rbinom()`) can be used to create the binary variable:
...
- The model found that $\beta_1$ (the effect size of 'x1') is `r summ_xy_mod_cat$coefficients[2, 1]` and that it's significantly different from 0 (p-value = `r summ_xy_mod_cat$coefficients[2, 4]`)

- The simulated values for the regression parameters can be compared to the summary of the `lm()` model to get a sense of the model precision:

    - $\beta_1$ was set to `r b1` ([line 132](#cb75-4)) and was estimated as `r round(summ_xy_mod_cat$coefficients[2, 1], 3)`
...
The interaction is easier to understand by plotting the data (don't worry too much about all the code):
...
- The model found that $\beta_1$ (the effect size of 'x1-b' to 'x1-a') is `r xy_summ_intr$coefficients[2, 1]` and that it's significantly different from 0 (p-value = `r xy_summ_intr$coefficients[2, 4]`) 

- The model found that $\beta_2$ (the effect size of 'x2') is `r xy_summ_intr$coefficients[3, 1]` and that it's significantly different from 0 (p-value = `r xy_summ_intr$coefficients[3, 4]`). This is actually the slope of the relation between x2 and y when x1 = 'a' 

- The model found that $\beta_3$ (the effect size of the interaction term 'x1 * x2') is `r xy_summ_intr$coefficients[4, 1]` and that it's significantly different from 0 (p-value = `r xy_summ_intr$coefficients[4, 4]`). This is the difference between the slopes of x2 *vs* y when x1 = 'a' and x2 *vs* y when x1 = 'b'.  

- The simulated values for the regression parameters can be compared to the summary of the `lm()` model to get a sense of the model precision:

    - $\beta_1$ was set to `r b1` ([line 175](#cb84-4)) and was estimated as `r round(xy_summ_intr$coefficients[2, 1], 3)`

    - $\beta_2$ was set to `r b2` ([line 176](#cb84-5)) and was estimated as `r round(xy_summ_intr$coefficients[3, 1], 3)`

    - $\beta_3$ was set to `r b3` ([line 177](#cb84-6)) and was estimated as `r round(xy_summ_intr$coefficients[4, 1], 3)`
...
<font size="5">Additional practice</font> 
...
- Now replace the error term with `error <- rexp(n = n, rate = 0.2)`. This is creating an error with an exponential distribution (so non-normal). This is supposed to be problematic for the inferential power of these models. Compare the estimates you got to the simulation values ('b0' and 'b1')
...
- Build a multiple regression model for this data (y ~ x1 + x2). You can use the same code as in [Adding more than 1 predictor: multiple regression](#adding-more-than-1-predictor-multiple-regression).

- How is inference affected by the presence of collinear predictors?

- Unbalanced data when having categories (i.e. some categories have way more observations than others) can be problematic for statistical inference. Modify the code in [this code block](#cb76-1) to simulate a highly unbalanced data set and check the precision of the model (hint: modify the 'prob' argument in [`x1 <- rbinom(n = n, size = 1, prob = 0.5)`](#cb76-11))
...
# Common statistical tests as special cases of linear regression
...
-[Causal Inference](http://bayes.cs.ucla.edu/jp_home.html)
